...
interface WorkSessionState {
  ...
  stopTimerWithAdjustment: (localId: string, coords?: Coordinates & { accuracy?: number }, adjustMinutes?: number) => Promise<void>;
  ...
  scheduleDelayedStart: (localId: string, localNome: string, minutes: number) => Promise<void>;
  
  // Agendar encerramento atrasado (usado pelo GeofenceAlert)
  scheduleDelayedStop: (localId: string, localNome: string, minutes: number, coords?: Coordinates & { accuracy?: number }) => void;
}
...
export const useWorkSessionStore = create<WorkSessionState>((set, get) => ({
  ...
  handleGeofenceEnter: async (localId, localNome, coords) => {
    ...
    if (sessaoAtual && sessaoAtual.status === 'ativa' && sessaoAtual.local_id !== localId) {
      logger.warn('workSession', 'BLOCKED: Already has ACTIVE session in another location', {
    ...
    if (pendingExit?.localId === localId) {
      ...
      logger.info('workSession', 'Cancelled pending exit - user returned quickly');
    ...
    const timeoutId = setTimeout(async () => {
      ...
      if (currentRegistro.sessaoAtual && currentRegistro.sessaoAtual.status === 'ativa') {
        logger.warn('workSession', 'Auto-start cancelled - session became active');
  ...
  handleGeofenceExit: async (localId, localNome, coords) => {
    ...
    if (!sessaoAtual || sessaoAtual.local_id !== localId || sessaoAtual.status !== 'ativa') {
      logger.debug('workSession', 'Not working at this location, ignoring exit');
      return;
    }
    
    logger.info('workSession', '🚪 Geofence EXIT - showing notification', { localId, localNome });
    
    // Mostrar notificação de saída
    const notificationId = await showGeofenceExitNotification(localId, localNome);
    ...
    const timeoutId = setTimeout(async () => {
      logger.info('workSession', '⏱️ Auto-STOPPING timer (30s timeout - no response)');
  ...
  handleNotificationAction: async (action, data) => {
    ...
    switch (action) {
      ...
      case 'continue':
        ...
        set({ pendingExit: null });
        logger.info('workSession', 'User chose to continue tracking outside fence');
        break;
  ...
  startTimer: async (localId, coords) => {
    ...
    if (sessaoAtual && sessaoAtual.status === 'ativa') {
      if (sessaoAtual.local_id === localId) {
        ...
        logger.error('workSession', 'BLOCKED: Cannot start timer - ACTIVE session in another location', {
          activeLocalId: sessaoAtual.local_id,
          attemptedLocalId: localId,
        });
      }
  ...
  stopTimerWithAdjustment: async (localId, coords, adjustMinutes = 0) => {
    logger.info('workSession', '⏹️ Stopping timer with adjustment', { localId, adjustMinutes });
  ...
  scheduleDelayedStart: async (localId: string, localNome: string, minutes: number) => {
    const delayNotifId = await scheduleDelayedStartNotif(localId, localNome, minutes);
    set((state) => {
      const newDelayed = new Map(state.delayedStarts);
  ...
  scheduleDelayedStop: (localId: string, localNome: string, minutesAgo: number, coords?: Coordinates & { accuracy?: number }) => {
    // NOTA: minutesAgo é PASSADO, ou seja, desconta do total
    // Ex: "Encerrar há 10 min" = encerra agora mas desconta 10min
    logger.info('workSession', `Stopping with ${minutesAgo} minutes ago adjustment`, { localId, localNome });
