...
export function exportarParaExcel({ sessoes, formato, incluirResumo = true }: ExportOptions) {
  // Preparar dados
  const dados = sessoes
  ...
  if (incluirResumo) {
    ...
    sessoes.filter(s => s.status === 'finalizada').forEach(s => {
      const nome = s.local_nome || 'N/A';
      resumoPorLocal.set(nome, (resumoPorLocal.get(nome) || 0) + (s.duracao_minutos || 0));
    });
    
    const dadosResumo = Array.from(resumoPorLocal.entries()).map(([local, minutos]) => ({
      'Local': local,
    ...
    const totalMinutos = Array.from(resumoPorLocal.values()).reduce((a, b) => a + b, 0);
    dadosResumo.push({
      'Local': 'TOTAL GERAL',
